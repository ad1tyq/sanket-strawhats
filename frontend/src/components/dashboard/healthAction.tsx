...
export default function HealthAction() {
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const result: AnalysisResult = await generateHealthActions();
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white p-4 rounded-lg shadow border">
              ...
              <p className="text-3xl font-bold text-yellow-600">
                {analysis.moderate_risk_patterns}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow border">
            ...
            {analysis.priority_summary?.immediate_actions?.length > 0 && (
              <div className="mb-6">
                ...
                <ul className="space-y-2">
                  {analysis.priority_summary.immediate_actions.map(
                    (action, index) => (
            ...
            {analysis.priority_summary?.high_priority_actions?.length > 0 && (
              <div className="mb-6">
                ...
                <ul className="space-y-2">
                  {analysis.priority_summary.high_priority_actions.map(
                    (action, index) => (
                      <li key={index} className="flex items-start">
            ...
            {analysis.priority_summary?.standard_actions?.length > 0 && (
              <div>
                ...
                <ul className="space-y-2">
                  {analysis.priority_summary.standard_actions.map(
                    (action, index) => (
                      <li key={index} className="flex items-start">
            ...
            <div className="grid gap-4">
              {analysis.patterns_detected.map((pattern, index) => (
                ...
                >
                  ...
                  {isDiseaseClusterPattern(pattern) && (
                    <>
                      <p className="text-sm">Disease: {pattern.disease}</p>
                      <p className="text-sm">
                        Total Cases: {pattern.total_cases}
                      </p>
                    </>
                  )}

                  {isLocationClusterPattern(pattern) && (
                    <>
                      <p className="text-sm">Location: {pattern.location}</p>
                      <p className="text-sm">
                        Total Cases: {pattern.total_cases}
                      </p>
                    </>
                  ...
                  {isHighRiskIndividualPattern(pattern) && (
                    <>
                      <p className="text-sm">Disease: {pattern.disease}</p>
                      <p className="text-sm">Location: {pattern.location}</p>
                      <p className="text-sm">Cases: {pattern.cases}</p>
                    </>
                  )}

                  {isNoDataPattern(pattern) && (
                    <p className="text-sm italic">{pattern.message}</p>
                  )}
                </div>
              ))}
            </div>
